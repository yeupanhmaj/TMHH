...
import { DecisionColums, DecisionDefine } from '../../models/decision';
...
export default class Decision extends React.Component {
  ...
  getmasterData() {
    DecisionService.getMasterData().then(
      result => {
  ...
  deleteRecords(ids) {
    DecisionService.deleteRecords(ids)
      .then(objRespone => {
        if (objRespone.isSuccess === true) {
          this.setState({
            confirmModal: false
  ...
  deleteRecord(id) {
    DecisionService.deleteRecord(id)
      .then(objRespone => {
        if (objRespone.isSuccess === true) {
          this.setState({
            confirmModal: false
  ...
  onModalSubmmitWithAttachFile(item, files) {
    item.dateIn = item.dateIn ? moment(new Date(item.dateIn)).format('YYYY-MM-DD') : moment(new Date()).format('YYYY-MM-DD');
    ...
    item.capitalID = item.bidCapital.value
  ...
  handelEditActionFromDataTable(item, type) {
    if (type === "delete") {
      let decisionCode = '';
      let decision = this.state.lstData.find((obj) => obj.decisionID.toString() === item.toString())
      if (item) {
        decisionCode = (decision).decisionCode;
      }
      let confirmContent = `Bạn có chắc muốn xóa quyết định chọn thầu : ${decisionCode}  ?`
      this.setState({ confirmModal: true, confirmContent, confirmType: type, deleteId: +(item) })
    } else {
      if (type === "deleteRecords") {
        let decisionCodes = [];
        let decisions = item.map((id) => {
          return this.state.lstData.find((obj) =>
            obj.decisionID === id
          )
        })
        if (decisions) {
          for (let record of (decisions)) {
            decisionCodes.push('  ' + (record).decisionCode);
          }
        }
        let confirmContent = `Bạn có chắc muốn xóa các quyết định chọn thầu : ${decisionCodes.toString()}  ?`
        this.setState({ confirmModal: true, confirmContent, confirmType: type, deleteIds: item })
      } else {
        let headerEditModal = "Chỉnh sửa quyết định chọn thầu";
        if (type === "insert") {
          headerEditModal = "thêm quyết định chọn thầu  mới";
  ...
  handleSearchDecisionCodeChangeInput(event) {
  ...
  render() {
    ...
    if (viewItem != undefined) {
      ItemDeatilsHeader = "Quyết định chọn thầu: " + viewItem.decisionCode;
    }
    ...
    return (
      <React.Fragment>
          ...
          <div style={{ width: '100%', height: 'auto', display: 'flex', flexWrap: 'wrap', top: '-10px' }}>
            <div className="childSearchWrap" >
              ...
              <input style={{
                ...
                type="text" className="form-control" name={'decisionCode'} value={this.state.searchState.decisionCode} onChange={
                  this.handleSearchDecisionCodeChangeInput.bind(this)
                } />
                 {this.validator.message('Mã quyết định', this.state.searchState.decisionCode, 'alpha_num_dash_space')}
            </div>
        ...
        <TableCustom
          ...
          keyColumn={CONST_FEATURE.Decision.KEY_COLUMN}
          columns={DecisionColums}
        ...
        <EditCreateItemModal
          ...
          keyColumn={CONST_FEATURE.Decision.KEY_COLUMN}
          isHasUploader={true}
          referFeatureType={CONST_FEATURE.Decision.feature}
        ></EditCreateItemModal>
        <ConfirmDialog
        ...
        {this.state.detailsItemModal == true &&
          <DecisionDetails
            ...
            onUpdateItem={(data) => {
              this.handleViewActionFromDataTable(data.decisionID)
            }}
          ></DecisionDetails>
        }
      </React.Fragment>
