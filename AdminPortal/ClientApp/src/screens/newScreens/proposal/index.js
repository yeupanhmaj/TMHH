...
import { ProposalColums, ProposalDefine, ProposalModel, ProposalTypeModel } from '../../../models/proposal';
...
export default class Proposa extends React.Component {
  _toDateCalendar;
  _fromDateCalendar;
  _tableCustom;
  ...
  getItem() {
    ItemService.GetAllItems().then((response) => {
      if (response.isSuccess == true) {
        ...
        this.setState({ itemOptions: response.data })
  ...
  handelEditActionFromDataTable(item, type) {
    if (type === "delete") {
      let proposalCode = '';
      let proposal = this.state.lstData.find((obj) => obj.proposalID === item)
      if (item) {
  ...
  render() {
    ...
    lstItem = [{ label: "Tất cả", value: '' }];
    if (this.state.itemOptions !== undefined) {
      for (var item of this.state.itemOptions) {
        let newItem = { label: item.itemName, value: item.itemName }

        lstItem.push(newItem)
            ...
            <div className="childSearchWrap">
              ...
              {lstItem !== undefined &&
                <Select
                  ...
                  styles={customStyles}
                  value={this.state.searchState.item}
                  onChange={this.handleChangeItemType.bind(this)}
                  options={lstItem}
                />
              }
            </div>
